...
------------------------------------
DECLARE @end int =3;
DECLARE @D DATE = '2018-03-14' , @Dend DATE = '2018-03-31';
DECLARE @willchangeCity NVARCHAR(100)  = N'Almonte'
...
select @Upper =  count (apilogid) from apilog where cscity = @willchangeCity and aclName='KHP'  and logdate >= @d AND logdate <DATEADD(DAY,1,@Dend)
SELECT @Random = ROUND(((@Upper - @Lower -1) * RAND() + @Lower), 0)
 if (@Random <1)  SELECT @Random = 1
...
(select top (@Random ) apilogid from apilog  where  cscity = @willchangeCity and aclName='KHP'   and    logdate >= @d AND logdate <DATEADD(DAY,1,@Dend)   order by apilogid) a
